...
class Llama3TS(nn.Module):
    def __init__(self, input_dim=1, seq_len=100, num_classes=4, device="cuda:0", llama_layers=6):
        ...
        # lstm
        self.lstm = nn.LSTM(input_size=self.llama3.config.hidden_size, hidden_size=256, num_layers=2, bidirectional=True, batch_first=True)
...
llama_model = Llama3TS(input_dim=1, seq_len=100, num_classes=5,
                     device="cuda:0", llama_layers=3).to(device)
