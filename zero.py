...
zero_dataset_name = args.zero_test
...
data_zero_2 = np.array(data_zero_1).astype("float32").reshape(-1, 100)
data_zero_2 = imputer.fit_transform(data_zero_2)
...
dataset_zero = TensorDataset(x_zero, y_zero)
train_loader = DataLoader(dataset, batch_size=32,
                          shuffle=True, pin_memory=False)
test_loader = DataLoader(dataset_test, batch_size=32,
                         shuffle=True, pin_memory=False)
zero_loader = DataLoader(dataset_zero, batch_size=32,
                         shuffle=True, pin_memory=False)
...
for epoch in range(num_epochs):
    ...
    test_loss /= len(zero_loader)
    test_acc = correct_predictions / total_predictions
    print(
        f"test_loss: {test_loss:.4f}, test_accuracy: {test_acc:.4f}")
    if test_loss < best_zero_loss:
        best_zero_loss = test_loss
        # best_model_state = model.state_dict()

    if test_acc > best_zero_acc:
        best_zero_acc = test_acc

    res_zero = {'zero/loss': test_loss, 'zero/acc': test_acc,
                'best/zero_loss': best_zero_loss, 'best/zero_acc': best_zero_acc}
    
    
    print(f"best/test_loss {best_test_loss}, best/test_acc: {best_test_acc}")
    res = {**res_train, **res_test, **res_zero}
    if args.wandb:
        wandb.log(res)
